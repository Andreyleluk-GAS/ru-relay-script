...
EOF
printf "${C_NC}"
echo -e "${C_PURPLE}${C_BOLD}  ✦ Super Relay Wizard v5.6.1 ✦${C_NC}"
...
case $MAIN_CHOICE in
    2) [ -f "$HISTORY_FILE" ] && (echo -e "\n${C_PURPLE}${C_BOLD}"; cat "$HISTORY_FILE"; echo -e "${C_NC}") || echo "История пуста"; exit 0 ;;
...
case $CHOICE in
    1)
        ...
        ;;
    2) read -p "Введите домен:порт : " L; EU_HOST=$(echo "$L" | awk -F: '{print $1}'); PORT=$(echo "$L" | awk -F: '{print $2}') ;;
    3) read -p "🌍 Целевой IP/Домен: " EU_HOST; read -p "🚪 ПОРТ: " PORT ;;
esac
...
(
    ...
    # ПРИНУДИТЕЛЬНО ОТКРЫВАЕМ ПОРТ 80 ДЛЯ ЗАГЛУШКИ
    iptables -I INPUT -p tcp --dport 80 -j ACCEPT
    if command -v ufw > /dev/null; then ufw allow 80/tcp > /dev/null; fi
...
if [ "$HAS_LINK" = true ]; then
    ...
    echo "[$(date +'%F %R')] Address: ${ENTRY} | Port: ${PORT}" >> "$HISTORY_FILE"
fi
